# ==============================================================================
# SEGMENT 1 OF 15: CORE DEPENDENCIES, CUSTOM UI CSS THEME & PLATFORM SLOGAN
# ==============================================================================
...
try:
    ...
    st.error("🚨 CRITICAL CORE MISCONFIGURED: 'main_engine.py' not found in root folder.")
    st.stop()
...
if is_valid_data and not full_validation_df.empty and not api_sync_triggered:
    ...
    for index, row in full_validation_df.iterrows():
        h_name, a_name = str(row["home_team"]).strip(), str(row["away_team"]).strip()
        ...
        if not historical_reference_df.empty:
            h_past = historical_reference_df[((historical_reference_df["home_team"] == h_name) | (historical_reference_df["away_team"] == h_name)) & (historical_reference_df["match_timestamp"] < current_match_time)]
            if not h_past.empty:
                days_diff = (current_match_time - h_past["match_timestamp"].max()).days
                calculated_home_rest_days = float(days_diff) if days_diff <= 14 else 5.0
                
            a_past = historical_reference_df[((historical_reference_df["home_team"] == a_name) | (historical_reference_df["away_team"] == a_name)) & (historical_reference_df["match_timestamp"] < current_match_time)]
            if not a_past.empty:
                days_diff = (current_match_time - a_past["match_timestamp"].max()).days
...
elif api_sync_triggered and total_fixtures > 0:
    st.sidebar.caption("⚡ Mode Status: API Manual Override Triggered")
    progress_bar = st.progress(0)
    for index, item in enumerate(target_fixtures):
        f_meta = item.get("fixture", {})
        teams, goals = item.get("teams", {}), item.get("goals", {})
        h_name, a_name = teams.get("home", {}).get("name", "N/A"), teams.get("away", {}).get("name", "N/A")
        current_match_time = pd.to_datetime(f_meta.get("date", datetime.datetime.now().isoformat()), errors='coerce')
...
if processed_execution_rows:
    ...
    st.success("⚡ SUCCESS! Your local spreadsheet records have been serialized flawlessly.")
...
if not working_pipeline_df.empty:
    ...
    for col in ["home_goals", "away_goals", "home_sot", "away_sot", "home_big_chances", "away_big_chances", "home_box_touches", "away_box_touches", "home_rest_days", "away_rest_days"]:
        if col in working_pipeline_df.columns:
            working_pipeline_df[col] = pd.to_numeric(working_pipeline_df[col], errors='coerce').fillna(0.0).astype('float64')
    uploaded_leagues = sorted(list(working_pipeline_df["league_country"].dropna().unique()))
else:
    st.info("📂 Data Control Room Active: Please upload your match CSV file to begin.")
    st.stop()
...
# ==============================================================================
# SEGMENT 8 OF 15: TIME-DECAY CURVE GRAPH & VENUE MOMENTUM TRACKERS
# ==============================================================================
...
with tab_pred:
    if not filtered_df.empty:
        ...
        if options:
            ...
            # ==============================================================================
# SEGMENT 9 OF 15: GLOBAL CONTAINER STRATEGIC OVERRIDES PANEL
# ==============================================================================

# --- RESET INDENTATION TRAK BOUNDARY SHIELD ---
# Left-aligned to the margin to permanently clear indentation traps
...
# ==============================================================================
# SEGMENT 10A OF 15: FLAT DYNAMIC MOTIVATION STANDINGS LOOPS & REMAP SHIELDS
# ==============================================================================
...
# ==============================================================================
                ...
                if home_tactical_style == "High-Possession Pressing" and away_tactical_style == "Fast Transition Counter-Attack":
                    ...
                    st.sidebar.warning("🛡️ Tactical Mismatch: Pressing host exposed to Counter-Attack.")
                ...
                # --- ASYMMETRIC SHORT-TERM BLIND SPOT MOTIVATION MATCH-TO-MATCH FILTER ---
                if live_standings_df is not None and not live_standings_df.empty:
                    # Enforce short-term motivation checks dynamically late in the season loop
                    if len(filtered_df) > (total_table_teams * 2 * 0.70):
                        if home_motivation_multiplier == 1.00 and away_motivation_multiplier == 1.15:
                            home_style_modifier *= 0.88 # Unmotivated home favorites drop output against desperate sides
                ...
                # ==============================================================================
# SEGMENT 11 OF 15: EXPECTED VALUE AUDITOR & GAME-STATE STRESS TESTER ENGINE
# ==============================================================================
                ...
                stress_rows = [
                    ...
                    {"Friction Scenario Profile": "Underdog Scores First (Early 0-1 Lead)", "Projected Draw (%)": f"{stress_away_lead_draw_prob * 100:.1f}%", "Projected Under 2.5 (%)": f"{stress_away_lead_under_prob * 100:.1f}%", "Risk Verdict": "🧱 Ultra Low-Block Inertia (Lock Under lines)"}
                ]
...
# ==============================================================================
                ...
                if graph_data_dict:
                    ...
                    # ==============================================================================
# SEGMENT 13 OF 15: MESSAGING RELAYS & CALIBRATED COUPLING INTERFACE
# ==============================================================================
...
with c_col_l:
    ...
    st.markdown(f"• **Dominant Threat Metrics Trace**: Home shots-on-target profile averages **{h_past_sot:.2f} SOT** per match relative to road parameters of **{a_past_sot:.2f} SOT**. Shot quality adjustments and schedule difficulty filters are fully compiled natively inside the core Poisson calculation loops.")
    # ==============================================================================
# SEGMENT 14 OF 15: BANKROLL PERFORMANCE LEDGER & CLV COUPLING MODULE
# ==============================================================================
with c_col_r:
...
with tab_tables:
    st.markdown(f"### Dynamic Standings Matrix & Outright Arbitrage: {selected_league_filter.upper()}")
    ...
    if not filtered_df.empty:
        ...
        if len(settled_check_df) == 0 or preseason_calibration_active:
            ...
            # Isolate participating squads securely
            ...
            # --- EXTRACT AND BUILD EDITABLE SQUAD PROFILE DATAFRAME ---
            squad_profile_rows = []
            for team in all_participating_teams:
                squad_profile_rows.append({
                    ...
                    "Corporate Manager Sack Floor (Min PPG)": 1.10, # Sacks manager if PPG drops below this floor mid-season
                    "Asymmetric Pitch Layout Width Index": 1.00,  # 1.00 is standard; lower than 0.95 means narrow ground
            ...
            edited_profile_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Corporate Manager Sack Floor (Min PPG)": st.column_config.NumberColumn("Sack PPG Floor", min_value=0.50, max_value=2.00, step=0.05, format="%.2f", help="Triggers mid-season positive bounce back if team drops below this PPG line."),
                    "Asymmetric Pitch Layout Width Index": st.column_config.NumberColumn("Pitch Width Index", min_value=0.85, max_value=1.15, step=0.05, format="%.2f", help="Lower than 0.95 compresses spatial passing wings by 7%."),
                    "Bookmaker Outright Odds": st.column_config.NumberColumn("Bookmaker Outright Odds", format="%.2f", min_value=1.01, step=0.5)
                },
                hide_index=True,
                use_container_width=True,
                key="outright_squad_ledger_v3"
            )

            # Map the user's manual inputs into local memory registries for low-latency calculations
            transfer_boost_map = {}
            departure_decay_map = {}
            depth_index_map = {}
            congestion_map = {}
            ...
            if edited_profile_df is not None:
                ...
                for team in all_participating_teams:
                    ...
                    # ==============================================================================
# SEGMENT 15B OF 15: MONTE CARLO TOURNAMENT SIMULATION CORE & REPORT MATRIX
# ==============================================================================

            # Execute the 1,000-iteration Monte Carlo tournament simulation pass loops safely
            ...
            if mock_schedule_fixtures:
                for iteration in range(1000):
                    ...
                    for index_f, fix in enumerate(mock_schedule_fixtures):
                        ...
                        # Increment tracking statistics
                        iteration_games_played[fix["home"]] += 1
                        iteration_games_played[fix["away"]] += 1
                        
                        # --- TOURNAMENT GROUP STAGE DEAD-RUBBER INTERCEPTOR ---
                        ...
                        # --- FACTOR 1: CORPORATE MANAGERIAL SACK-VELOCITY BOUNCE LOOP ---
                        # Sacks underperforming managers mid-season and triggers a +10% positive mean-reversion bounce
                        ...
                        # --- FACTOR 3: ASYMMETRIC TURF/PITCH SYMMETRY LAYOUT DEFICIT ---
                        # Narrow ground width compresses spatial transition passing lanes by 7%
                        home_pitch_modifier = 1.00
                        if pitch_width_map.get(fix["home"], 1.00) < 0.95:
                            home_pitch_modifier = 0.93
                        ...
                        # Dynamic Attrition Depth Filter
                        ...
                        # Multi-Cup Schedule Congestion Modifier
                    ...
                    # Apply a Historical Points Ceiling Cap to handle extreme mathematical variance anomalies
                    for team in iteration_points_registry:
                        if iteration_points_registry[team] > 100:
                            iteration_points_registry[team] = 100
            ...
            # Tabulate outputs probability distributions and calculate Outright EV options pricing
